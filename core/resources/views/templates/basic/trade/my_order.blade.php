        ...
        `;
            ...
            @if (App::getLocale() != 'ar')
                <tr data-order-id="${order.id}">
                    ...
                    <td class="text-center p-2"> <span class="${profitClass}">${parseFloat(total_price).toFixed(decimalCount)}</span></td>
        ...
        `;
            ...
            function removeTrailingZeros(number) {
                ...
                if (Number.isInteger(trimmedNumber)) {
                    return (trimmedNumber - Math.floor(trimmedNumber)) !== 0 ? trimmedNumber.toFixed(2) :
                        trimmedNumber.toFixed();
                }
                ...
                function makeAjaxRequest(order) {
                    ...
                    let total_price = parseInt(order.order_side) === 2 ?
                        formatWithPrecision(((parseFloat(order.rate) - parseFloat(current_price.replace(/,/g,
                            ''))) * lotEquivalent)) :
                        formatWithPrecision(((parseFloat(current_price.replace(/,/g, '')) - parseFloat(order
                            .rate)) * lotEquivalent));
...
@push('style')
    <style>
        ...
        @media (max-width: 600px) {
            ...
            .trading-table {
                overflow-x: auto;
                /* Allows scrolling for tables on small screens */
            }
        ...
        [data-theme=light] .clickable-row[aria-expanded="true"],
        [data-theme=light].history-collapse.show {
            background-color: #ffffff;
        }

        [data-theme=light] .clickable-row[aria-expanded="true"] .h-label,
        [data-theme=light] .clickable-row[aria-expanded="true"] strong,
