...
class AdminController extends Controller
{
    ...
    public function managelots(){
        $pageTitle = 'Manage Lots Volume';
        $lots = LotManager::all();
        return view('admin.managelots', compact('pageTitle', 'lots'));
    }

    public function savelots(Request $request){
        $validated = $request->validate([
            'lot_volume.*'  => 'required|numeric',
            'lot_value.*'   => 'required|numeric',
            'selected_lot'  => 'nullable|integer', // The selected lot (can be null if none is selected)
        ]); 

        LotManager::truncate();

        foreach ($request->lot_volume as $index => $volume) {
            $lot = LotManager::create([
                'lot_volume' => $volume,
                'lot_value' => $request->lot_value[$index],
                'selected' => $request->selected_lot == $index, // If this is the selected lot
            ]);
        }

        return response()->json(['success' => 'Lots saved successfully!']);
    }

    public function deletelot($id){
    // Find the lot by id and delete it
        $lot = LotManager::findOrFail($id);
        $lot->delete();

        return response()->json(['success' => 'Lot deleted successfully!']);
    }

    public function togleFee(){
        $pageTitle = 'Manage Fee';
        $fee = Fee::first();
        return view('admin.managefee', compact('pageTitle', 'fee'));
    }
    ...
    public function feeSave(Request $request){
        ...
        return response()->json(['success' => 'Fee status updated successfully!']);
